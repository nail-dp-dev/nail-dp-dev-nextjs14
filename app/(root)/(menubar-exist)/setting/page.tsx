...
export default function SettingPage() {
  ...
  return (
    <div className="SettingContainer h-full w-full overflow-hidden">
      ...
      {/* 버튼 메뉴 */}
      <div
        className={`${isCategoryBar === 'subscribe' ? 'mb-[20px]' : 'mb-[26px]'} `}
      >
        {isCategoryBar === 'subscribe' && (
          <div className="flex h-[32px] w-full gap-[20px] text-[14px] font-[700]">
            <button
              onClick={(e) => {
                MenuBarClick(e, '결제수단 비츠');
              }}
              className={`${isMenu === '결제수단 비츠' ? 'hashtag-layout button-tr-tf button-transition bg-purple text-white active:bg-darkPurple' : 'hashtag-layout hashtag-hover-active button-tr-tf button-transition hover bg-lightGray'}`}
            >
              결제 수단 • 비츠
            </button>
            <button
              onClick={(e) => {
                MenuBarClick(e, '결제 내역 보기');
              }}
              className={`${isMenu === '결제 내역 보기' ? 'hashtag-layout button-tr-tf button-transition bg-purple text-white active:bg-darkPurple' : 'hashtag-layout hashtag-hover-active button-tr-tf button-transition bg-lightGray'}`}
            >
              결제 내역 보기
            </button>
            <button
              onClick={(e) => {
                MenuBarClick(e, '나의 구독');
              }}
              className={`${isMenu === '나의 구독' ? 'hashtag-layout button-tr-tf button-transition bg-purple text-white active:bg-darkPurple' : 'hashtag-layout hashtag-hover-active button-tr-tf button-transition bg-lightGray'}`}
            >
              나의 구독
            </button>
          </div>
        ...
        {isCategoryBar === 'myAccount' && (
          <div className="flex h-[32px] w-full gap-[20px] text-[14px] font-[700]">
            <button
              onClick={(e) => {
                MenuBarClick(e, '내 계정 관리');
              }}
              className={`${isMenu === '내 계정 관리' ? 'hashtag-layout button-tr-tf button-transition bg-purple text-white active:bg-darkPurple' : 'hashtag-layout hashtag-hover-active button-tr-tf button-transition bg-lightGray'}`}
            >
              내 계정 관리
            </button>
            <button
              onClick={(e) => {
                MenuBarClick(e, '간편 로그인 연동 • 해제');
              }}
              className={`${isMenu === '간편 로그인 연동 • 해제' ? 'hashtag-layout button-tr-tf button-transition bg-purple text-white active:bg-darkPurple' : 'hashtag-layout hashtag-hover-active button-tr-tf button-transition bg-lightGray'}`}
            >
              간편 로그인 연동 • 해제
            </button>
          </div>
        ...
        {isCategoryBar === 'alarm' && (
          <div className="flex h-[32px] w-full justify-between gap-[20px] text-[14px] font-[700]">
            <button
              onClick={(e) => {
                MenuBarClick(e, '알림 상세 설정');
              }}
              className={`${isMenu === '알림 상세 설정' ? 'hashtag-layout button-tr-tf button-transition bg-purple text-white active:bg-darkPurple' : 'hashtag-layout hashtag-hover-active button-tr-tf button-transition bg-lightGray'}`}
            >
              알림 상세 설정
            </button>
            ...
            <div className="mr-[16px] flex items-center">
              <p
                className={`mr-[20px] text-[18px] font-[400] text-textDarkPurple`}
              >
                {`모든 알림 ${isAlarm ? '끄기' : '켜기'}`}
              </p>
              <div className="flex">
                <div
                  className={`flex h-[26px] w-[53px] cursor-pointer items-center  rounded-full border-2 border-lightPurple/80 transition-all duration-300 ${isAlarm ? 'bg-purple' : 'bg-navMenuBotSolidGray '}`}
                  onClick={alarmToggle}
                >
                  <div
                    className={`h-[24px] w-[24px] transform rounded-full bg-white shadow-md transition-transform duration-300 ${isAlarm ? 'translate-x-[30px]' : ''}`}
                  />
                </div>
              </div>
            </div>
        ...
        {isCategoryBar === 'chat' && (
          <div className="flex h-[32px] w-full gap-[20px] text-[14px] font-[700]">
            <button
              onClick={(e) => {
                MenuBarClick(e, '채팅방 보기 설정');
              }}
              className={`${isMenu === '채팅방 보기 설정' ? 'hashtag-layout button-tr-tf button-transition bg-purple text-white active:bg-darkPurple' : 'hashtag-layout hashtag-hover-active button-tr-tf button-transition bg-lightGray'}`}
            >
              채팅방 보기 설정
            </button>
            <button
              onClick={(e) => {
                MenuBarClick(e, '채팅 차단 목록');
              }}
              className={`${isMenu === '채팅 차단 목록' ? 'hashtag-layout button-tr-tf button-transition bg-purple text-white active:bg-darkPurple' : 'hashtag-layout hashtag-hover-active button-tr-tf button-transition bg-lightGray'}`}
            >
              채팅 차단 목록
            </button>
          </div>
        ...
        {isCategoryBar === 'theme' && (
          <div className="flex h-[32px] w-full gap-[20px] text-[14px] font-[700]">
            <button
              onClick={(e) => {
                MenuBarClick(e, '화면 테마');
              }}
              className={`${isMenu === '화면 테마' ? 'hashtag-layout button-tr-tf button-transition bg-purple text-white active:bg-darkPurple' : 'hashtag-layout hashtag-hover-active button-tr-tf button-transition bg-lightGray'}`}
            >
              화면 테마
            </button>
            <button
              onClick={(e) => {
                MenuBarClick(e, '채팅방 테마');
              }}
              className={`${isMenu === '채팅방 테마' ? 'hashtag-layout button-tr-tf button-transition bg-purple text-white active:bg-darkPurple' : 'hashtag-layout hashtag-hover-active button-tr-tf button-transition bg-lightGray'}`}
            >
              채팅방 테마
            </button>
            <button
              onClick={(e) => {
                MenuBarClick(e, '스토어 테마');
              }}
              className={`${isMenu === '스토어 테마' ? 'hashtag-layout button-tr-tf button-transition bg-purple text-white active:bg-darkPurple' : 'hashtag-layout hashtag-hover-active button-tr-tf button-transition bg-lightGray'}`}
            >
              스토어 테마
            </button>
          </div>
        )}
        {isCategoryBar === 'language' && (
          <div className="flex h-[32px] w-full gap-[20px] text-[14px] font-[700]">
            <button
              onClick={(e) => {
                MenuBarClick(e, '현재 설정');
              }}
              className={`${isMenu === '현재 설정' ? 'hashtag-layout button-tr-tf button-transition bg-purple text-white active:bg-darkPurple' : 'hashtag-layout hashtag-hover-active button-tr-tf button-transition bg-lightGray'}`}
            >
              현재 설정
            </button>
          </div>
      ...
      {/* 여기가 변경 */}
      {/* 구독결제 관리 */}
      {isCategoryBar === 'subscribe' && isMenu === '결제수단 비츠' && (
        <div className="flex flex-col">
          <div className="mb-[9px] flex items-center justify-between">
            ...
            <div className="mr-[16px] flex items-center">
              <div className="flex">
                <div
                  className={`flex h-[26px] w-[53px] cursor-pointer items-center  rounded-full border-2 border-lightPurple/80 transition-all duration-300 ${isOn ? 'bg-purple' : 'bg-navMenuBotSolidGray '}`}
                  onClick={handleToggle}
                >
                  <div
                    className={`h-[24px] w-[24px] transform rounded-full bg-white shadow-md transition-transform duration-300 ${isOn ? 'translate-x-[30px]' : ''}`}
                  />
                </div>
              </div>
              <p
                className={`${isOn ? 'text-purple' : 'text-darkGray'} ml-[10px] text-[14px] font-[700]`}
              >
                나의 비츠 잔액 숨기기
              </p>
            </div>
          ...
          <div className="relative mb-[40px] flex h-[110px] w-full rounded-lg border-2 border-purple">
            <div className="ml-[34px] flex items-center gap-[20px]">
              <Bits className="h-[44px] w-[44px]" />
              <p className="text-[21px] font-[700] text-darkPurple">
                {'1,000'} 비츠
              </p>
            </div>
            <button className="absolute right-[16px] top-[16px] rounded-lg bg-lightPurple/80 px-[12px] py-[4px] text-[14px] font-[700] text-darkPurple hover:text-purple">
              자동 충전
            </button>
          </div>
          <div>
            ...
            <div className="flex h-[215px] w-full justify-center rounded-lg border-2 border-purple">
              <div className="flex w-full flex-col items-center">
                <p className="pb-[40px] pt-[35px] text-[18px] font-[500] text-darkPurple">
                  등록된 결제 수단이 없습니다.
                </p>
                <button className="relative flex h-[55px] w-[260px] items-center justify-center rounded-full bg-lightGray text-[18px] font-[700] text-darkPurple shadow-md">
      ...
      {isCategoryBar === 'alarm' && (
        <div className="flex h-full w-full flex-col">
          <div className="max-h-full text-[18px]">
            <div className="flex h-full flex-col items-center overflow-y-scroll rounded-lg border-2 border-purple transition-all">
      ...
      {isCategoryBar === 'chat' && isMenu === '채팅방 보기 설정' && (
        <div className="flex flex-col rounded-lg border-2 border-purple">
          <div
            className={`flex min-h-[55px] w-full items-center justify-between text-[14px] font-[700] ${isChatSetting ? 'rounded-lg border-b-2 border-lightPurple' : ''}`}
          >
              ...
              return (
                isChatState !== item.name && (
                  <button
                    ...
                    className={`flex h-[55px] w-full items-center justify-between rounded-lg text-[14px] font-[700] hover:bg-lightPurple ${isLastItem ? '' : 'rounded-lg border-b-2 border-lightPurple'} `}
                  >
      ...
      {isCategoryBar === 'theme' && isMenu === '화면 테마' && (
        <div className="flex flex-col rounded-lg border-2 border-purple">
          <div
            className={`flex min-h-[55px] w-full items-center justify-between text-[14px] font-[700] ${isScreenTheme ? 'rounded-lg border-b-2 border-lightPurple' : ''}`}
          >
            <p className="pl-[34px]">
              ...
              return (
                ((darkMode && item.name === '라이트 모드') ||
                  ...
                  <button
                    key={index}
                    onClick={modeToggle}
                    className={`flex h-[55px] w-full items-center justify-between rounded-lg text-[14px] font-[700] hover:bg-lightPurple ${
                      isLastItem
                        ? ''
      ...
      {isCategoryBar === 'language' && isMenu === '현재 설정' && (
        <div className="flex flex-col rounded-lg bg-lightPurple">
          <div
            className={`flex min-h-[55px] w-full items-center justify-between text-[14px] font-[700]`}
          >
