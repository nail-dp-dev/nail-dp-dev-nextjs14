            ...
            </button>
            {
              data.data.profileUrls.map((item:any, index:any) => (
                <button
                  ...
                  onClick={(e)=>handleChangeProfileImage(e,item)}
                >
                  <Image src={item.startsWith('http://') || item.startsWith('https://') || item.startsWith('/') ? item :`${process.env.NEXT_PUBLIC_CLOUDFRONT_URL}/${item}`} width={30} height={30} alt={'profileImage'} style={{objectFit: 'cover', width: '100%', height: '100%', maxWidth: '30px', maxHeight: '30px'}} quality={100} sizes='100vw' className='rounded-full '></Image>  
                </button>
              )
            )}
          </div>
